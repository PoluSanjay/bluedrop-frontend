...
const Contact = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-16">
      <div className="hero-gradient py-14 px-4 text-white text-center">
        <h1 className="text-4xl font-black mb-3">Contact Us</h1>
        <p className="text-blue-100 max-w-md mx-auto">We're here to help! Reach us through any of the channels below.</p>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 -mt-6 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
            <h2 className="font-bold text-slate-800 text-xl mb-5">Get In Touch</h2>
              ...
              {contactInfo.map(info => (
                <div key={info.label} className="flex gap-4 items-start">
                  ...
                  <div>
                    ...
                    {info.href ? (
                      <a href={info.href} className="font-semibold text-slate-800 hover:text-blue-600 transition-colors">
                        {info.value}
                      </a>
                    ) : (
                      <p className="font-semibold text-slate-800 whitespace-pre-line">{info.value}</p>
                    )}
                  </div>
                </div>
              ))}
            ...
            <div className="mt-6 pt-6 border-t border-slate-100">
              ...
              <div className="flex flex-col gap-2">
                <a href="https://wa.me/917842809475?text=Hi, I need help with my water purifier"
                  ...
                  <div>
                    <p className="font-semibold text-green-800 text-sm">WhatsApp Support</p>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            <div className="h-full min-h-64 bg-blue-50 flex flex-col items-center justify-center text-center p-8">
              <div className="text-5xl mb-4">🗺️</div>
              <h3 className="font-bold text-slate-800 mb-2">Find Us on Map</h3>
              <p className="text-slate-500 text-sm mb-4">42, Water Tower Road, Kukatpally, Hyderabad</p>
              <a href="https://maps.google.com/?q=Kukatpally+Hyderabad"
                target="_blank" rel="noreferrer"
                className="btn-primary text-sm">Open in Google Maps</a>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
          <h2 className="font-bold text-slate-800 text-xl mb-4">Service Areas in Hyderabad</h2>
          <div className="flex flex-wrap gap-2">
            {['Kukatpally', 'Madhapur', 'Gachibowli', 'Miyapur', 'KPHB', 'Kondapur', 'Hitec City', 'Begumpet', 'Jubilee Hills', 'Banjara Hills', 'LB Nagar', 'Dilsukhnagar', 'Uppal', 'Secunderabad', 'Ameerpet', 'Mehdipatnam', 'Manikonda', 'Narsingi', 'Attapur', 'Tolichowki'].map(area => (
              <span key={area} className="px-3 py-1.5 bg-blue-50 text-blue-700 rounded-full text-sm font-medium hover:bg-blue-100 transition-colors cursor-default">
                📍 {area}
              </span>
            ))}
          </div>
